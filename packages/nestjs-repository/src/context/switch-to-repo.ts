 ...
 *
 * - **First call** (entity not yet set): sets entity directly on the
 *   context and initializes `hooks` to `[]` if absent. Returns the context.
...
export function switchToRepo(
  ...
  if (!('hooks' in ctx)) {
    Object.defineProperty(ctx, 'hooks', {
      value: [],
      enumerable: true,
      configurable: false,
      writable: false,
