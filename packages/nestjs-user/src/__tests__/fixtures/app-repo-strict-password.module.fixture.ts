...
/**
 * Same shape as `AppRepoModuleFixture`, but with `minPasswordStrength`
 * forced to `VeryStrong` regardless of `NODE_ENV` — regression coverage for
 * #469 needs a password that reliably fails strength validation.
 */
@Module({
  imports: [
    ...
    PasswordModule.forRoot({
      settings: { minPasswordStrength: PasswordStrengthEnum.VeryStrong },
    }),
